$(document).ready(function(){
    ...
    function load_content(){
        ...
        if(active == "select")
        {
           ...
           search_TVGenre(pageNumber,$('#searchingTV').val());
           $(".loadArea").empty();
        }
    }

    function search_movie(data){
      $("#frontPage").fadeOut();
        $.ajax({
            url: "/findTitle/" + data,
            type: "GET",
    ...
    function search_tv(data){
      $("#frontPage").fadeOut();
        $.ajax({
            url: "/findTvTitle/" + data,
            type: "GET",
            success: function(rsp)
            {
                load_cache(rsp.data);
    ...
    function search_TVGenre(page,data){
      $("#frontPage").fadeOut();
        $.ajax({
            url: "/findTvGenre/"+page+"/"+data,
            type: "GET",
            success: function(rsp)
            {
                load_cache(rsp.data);
    ...
    function create_layout(data,movieData){
        ...
        for( i = 0;i<movieData.length;i++)
        {
           if(movieData[i]["poster_path"] != null)
           {
             ...
             +'<h2>'+status+'</h2>'
             +'<h3 class="info" id="'+movieData[i]["id"]+'"><button class = "hidden-sm" type="button"data-toggle="modal" data-target="#myModal">Overview</button></h3>'
             +'</div>'
             +'</div>'
             +'</div>'
    ...
    $(document).ready(function () {
     ...
     $(document).ready(function () {
        ...
        $('#searchingTV').change(function () {
            ...
            search_TVGenre(pageNumber,$(this).val());
        })
    });
    ...
    $("#submit").click(function(evt) {
       ...
       search_movie(searchBarVal);

    });

    
    $("#tvSubmit").click(function(evt3) {
        var searchBarVal = $("#searchTv").val();
        evt3.preventDefault();
        //console.log("searchbarVal:",searchBarVal);
        active = "searchBar";
        pageNumber = 0;
